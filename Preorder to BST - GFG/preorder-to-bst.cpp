...
public:
    ...
    int findPosition(vector<int> preorder,int elem,int size)
    {
        for(int i=0;i<size;i++)
         if(preorder[i]==elem) return i;
        return -1;
    }
    
    Node* solve(int pre[],vector<int> inorder,int &index,int s,int e,int size)
    {
        if(index>=size || s>e) return NULL;
        int elem=pre[index++];
        
        int pos=findPosition(inorder,elem,size);
        Node* root=new Node();
        root->data=elem;
        root->left=solve(pre,inorder,index,s,pos-1,size);
        root->right=solve(pre,inorder,index,pos+1,e,size);
        return root;
    }
    Node* post_order(int pre[], int size)
    {
       ...
       int index=0;
       Node* ans=solve(pre,inorder,index,0,size-1,size);
       return ans;
    }
};
